...
components/
  ...
  matches/          visibility-toggle.tsx (público/privado, se usa al crear y en el detalle), match-visibility-switch.tsx, invite-players.tsx (organizador invita jugadores a un partido privado: sección plegable "Invitar a un grupo" por grupo — evita apilar un botón por grupo — + panel de amigos con buscador/lista con scroll, multi-selección y "invitar a todos mis amigos", combinando `getMyFriends()`/`getMyGroups()` con `searchUsersAction`)
