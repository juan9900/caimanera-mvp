...
## Route groups: producto vs. admin

`app/` tiene un único `layout.tsx` raíz (`<html>`/`<body>`, fuentes, `ServiceWorkerRegistration`) y dos route groups debajo, que no aparecen en la URL:

- **`app/(app)/`** — todo el producto (home, partidos, canchas, mapa, perfil, auth, etc.). Su `layout.tsx` monta el chrome del producto: `SiteHeader` + `BottomNav` (tab bar), tema oscuro "High-Velocity".
- **`app/(admin)/`** — el panel `/admin` completo. Su `layout.tsx` centraliza las guardias (`verifySession`/`getCurrentUserProfile`/`getIsAdmin`, antes copiadas en cada página) y monta un shell propio — `AdminSidebar` + `AdminTopbar`, tema claro deliberadamente distinto del producto — sin `SiteHeader` ni `BottomNav`.

Ninguno de los dos define su propio `<html>`/`<body>` (no son "root layouts" independientes en el sentido de Next), así que navegar entre `/` y `/admin` no dispara un full page reload.
...
app/
  actions/          Server Actions ("use server") — toda escritura a la DB pasa por aquí
    ...
    courts.ts       agregar/editar cancha (admin, incluye `is_public`; ya NO incluye `is_official`/`sponsored_until`, ver `billing.ts` abajo); `createPendingCourt` (cualquier usuario) agrega un lugar al instante como cancha `verified: false`; `verifyCourt`/`deletePendingCourt` (admin) verifican o eliminan un pendiente
    billing.ts      admin-only: `setCourtPlan`/`cancelCourtPlan` (fija/renueva/cancela el plan pagado de una cancha en `court_subscriptions`), `registerCourtPayment` (bitácora de cobros manuales en `court_payments`), `addCourtManager`/`removeCourtManager` (vincula/desvincula un usuario como dueño de una cancha en `court_managers`) — ver "Planes y cobro de canchas" abajo
  ...
  (admin)/          route group del panel — `layout.tsx` centraliza las guardias admin + el shell (sidebar/topbar, ver "Route groups" arriba)
    admin/            ve usuarios, partidos, canchas, lugares pendientes de verificar, métricas; tablas reales (`DataTable`/`SearchableTable`) en vez de listas, primitivos en `components/admin/ui/` (ver abajo)
...
lib/
  ...
  hooks/use-click-outside.ts  cierra un menú/dropdown al hacer click afuera — compartido por `header-nav.tsx` (Social, Cuenta) y `admin-sidebar.tsx` (slide-over móvil)
...
components/
  ...
  admin/            `admin-sidebar.tsx`/`admin-topbar.tsx` (shell del panel: nav lateral fija en desktop, slide-over en móvil vía un `CustomEvent` disparado por el botón hamburguesa de la topbar — ambos son client components separados, montados desde `app/(admin)/layout.tsx`), `court-plan-card.tsx` (tres tarjetas de `/admin/canchas/[id]/editar`: plan vigente + form fijar/renovar, form registrar cobro manual, lista de managers vinculados), `court-manager-search.tsx` (buscador de usuarios para vincular como dueño de una cancha, reutiliza `searchUsersAction` de `app/actions/friends.ts`, debounce de 300ms), `canchas-table.tsx`/`usuarios-table.tsx` (client, envuelven `SearchableTable` con sus columnas — las páginas server de `/admin/canchas` y `/admin/usuarios` solo pasan filas serializables; las funciones `render` de las columnas no pueden cruzar el límite server→client como prop) — ver "Planes y cobro de canchas" abajo
  admin/ui/         primitivos del panel de admin (tema claro, deliberadamente separado del design system oscuro del producto): `page-header.tsx`, `card.tsx`, `stat-card.tsx`, `data-table.tsx` (+ `searchable-table.tsx`, que le agrega un filtro de texto en cliente), `badge.tsx`, `button.tsx`, `field.tsx` (`Field`/`Input`/`Select`/`Textarea`), `confirm-submit.tsx` (botón de submit con `confirm()` antes de dejar pasar el form — usado en "Eliminar" de `/admin/sugerencias`)
  courts/           inputs de formulario de cancha (comodidades + deportes + checkbox `isPublic` — ya NO incluye `isOfficial`, ver "Planes y cobro de canchas" abajo), court-picker.tsx (selector desplegable de cancha al crear partido), suggest-court-form.tsx (`AddPlaceInline` — sección expandible embebida "¿No aparece tu lugar?", sin navegar a otra página; nombre + `LocationPointPicker`, llama `createPendingCourt`, que crea la cancha `verified: false` al instante y la devuelve para que el llamador la seleccione — usada en `MapExperience` y en los pickers de crear/editar partido, ver abajo), sport-chip.tsx (chip de un deporte — ícono siempre, etiqueta opcional vía `showLabel` — compartido por el filtro del home, `/mapa` y el selector de deporte al crear partido), rating-stars.tsx (estrella/fila de estrellas + promedio y cantidad, solo lectura — variante `compact` para el badge del pin y las tarjetas de `/mapa`, `detail` para la ficha de la cancha; "Sin calificaciones" si `count === 0`), rate-court.tsx (picker interactivo de 1-5 estrellas en la ficha de la cancha, llama a `rateCourt`), court-hero.tsx (hero full-bleed con foto/degradado + badge "Oficial" para la ficha de canchas `is_official`, con fallback de color plano si no hay fotos), amenities-showcase.tsx (grid de comodidades en tiles con ícono, la versión "premium" de las mismas `AMENITIES` que `amenity-icons.tsx` renderiza como chips compactos en las tarjetas), official-upsell.tsx (tarjeta "hazte partner oficial" mostrada en la ficha de canchas no oficiales en vez de WhatsApp/Reserva/Comodidades/Fotos)
  ...
  map/map-shared.tsx  tiles oscuros (`DarkTiles`) + `buildCourtIcon` (pin oscuro: cero deportes → pin genérico, uno → badge redondo con su ícono, dos o más → píldora horizontal con hasta 3 íconos — priorizando los deportes favoritos del usuario (`opts.preferredSports`, de `profile.sport_preferences`) que la cancha ofrece y rellenando con el resto — más un "+N" si la cancha ofrece más de 3, ver `pickPinSports`; badge de check en la esquina cuando `opts.official`/`is_official`, mismos colores que el chip "Oficial" del resto de la app; badge de globo cuando `opts.public`/`is_public` en vez — oficial tiene prioridad si una cancha fuera ambas, así el pin nunca apila dos badges) + `CourtPopupContent` (contenido del popup del pin: nombre, `RatingStars` compacto si se pasan `ratingAvg`/`ratingCount` (solo el popup de `/mapa` los pasa), chips de deporte si son varios, link "Ver cancha" opcional) — usados por todos los mapas del repo
...
docs/
  mvp-spec.md       qué es el producto, qué SÍ y qué NO construir, modelo de datos
  home-ui-spec.md   detalle de implementación del home
  business-model.md modelo de negocio: planes/precios para canchas, reglas de torneos, roadmap técnico de monetización
```
...
- `courts` — cancha. `verified` distingue una cancha del catálogo curado (`true`, valor por defecto) de un lugar que un usuario agregó al vuelo (`false`) con `createPendingCourt` (`app/actions/courts.ts`, cualquier sesión — botón "¿No aparece tu lugar?"/`AddPlaceInline` en el mapa de exploración y en los pickers de crear/editar partido). Una cancha `verified: false` es utilizable de inmediato por quien la agregó (por ejemplo para su partido), pero `getCourts()`/`getCourt()` (`lib/auth/dal.ts`) solo la devuelven a su creador (`added_by`) y a los admins — para el resto no existe hasta que un admin la verifica en `/admin/sugerencias` (`getPendingCourts()`, `verifyCourt`/`deletePendingCourt`); quien se une a un partido que la usa igual ve su ubicación a través del partido. No confundir con `is_official`, que distingue cancha "de verdad" (con ficha, fotos, aparece en explorador y en el home) de una ubicación disponible solo para elegir al crear partido. **`is_official` y `sponsored_until` son un agregado derivado, no se editan a mano** desde la migración `court_billing_plans` (ver "Planes y cobro de canchas" abajo): reflejan el plan pagado vigente de `court_subscriptions`, y el único que los escribe es el trigger `court_subscriptions_sync`/el cron diario `sync_court_plan_flags`. `is_public` es independiente de `is_official` (una cancha puede ser ambas, ninguna, o solo una): marca un lugar gratuito/abierto (ej. cancha de plaza o parque) y muestra la etiqueta "Lugar público" en el pin/popup del mapa (`buildCourtIcon`/`CourtPopupContent`, ver `map/map-shared.tsx` abajo), en la ficha de la cancha, en la tarjeta de `/mapa` y en el título del partido cuando se juega ahí (`app/partidos/[id]/page.tsx`). Se marca con un checkbox en el form de admin (`CourtSponsorshipFields`). En la ficha de la cancha (`app/canchas/[id]/page.tsx`) `is_official` además gatea la UI: solo las oficiales muestran el hero de foto (`CourtHero`), WhatsApp/Reservar (`CourtContactActions`), la sección de comodidades (`AmenitiesShowcase`) y la galería de fotos — las no oficiales ven una versión mínima (nombre, mapa, rating, "Cómo llegar") + una tarjeta de upsell (`OfficialUpsell`) invitando a hacerse partner. `sponsored_until`/`sponsor_priority` son el nivel pago (destacado en home) y es un concepto aparte de `is_official` (una cancha oficial puede o no estar patrocinada; el badge "Patrocinado" en la ficha depende solo de `sponsored_until`). `sports` (array de claves de `lib/courts/sports.tsx`) determina qué deportes ofrece — se elige con checkboxes en el form de admin (`CourtSponsorshipFields`) — y los íconos del pin en el mapa (`buildCourtIcon`, ver arriba). `address` es texto libre mostrado en las tarjetas de `/mapa`. `schedule` es texto libre para la ficha de la cancha; `opens_at`/`closes_at` (hora) + `open_days` (array de días con la convención `Date.getDay()` de JS: 0=domingo..6=sábado) son el horario estructurado usado para calcular "abierto hoy" — ver `lib/courts/hours.ts`. Por ahora el badge de horario en el selector de crear partido solo se muestra para canchas `is_official` (hoy, Cantera — que ofrece fútbol, pádel y futsal). `rating_avg`/`rating_count` son un agregado desnormalizado mantenido por un trigger (`recompute_court_rating`) sobre `court_ratings` — se lee directo de `courts` (sin join) en todos los mapas y en la ficha de la cancha. Aplica tanto a canchas oficiales como no oficiales; en las tarjetas de `/mapa` el rating reemplaza al cuadro de imagen en las no oficiales (que nunca tienen fotos).
...
Regla de negocio clave que se repite en el código: **nunca hay pagos dentro de la app** — todo `payment*` en `matches` (banco, teléfono, cédula, monto) es solo información para coordinar el pago externo (Pago Móvil), no una transacción real. Lo mismo aplica a la confirmación de pago descrita abajo: la referencia es solo un dato para que el organizador verifique el Pago Móvil por fuera. La misma regla aplica al cobro a canchas descrito abajo: `court_payments` es solo bitácora, el cobro real ocurre por fuera (Pago Móvil/Zelle).

## Planes y cobro de canchas

Modelo de negocio completo en `docs/business-model.md`; esto es solo el resumen técnico. Migración: `court_billing_plans` (+ `court_billing_cron_and_backfill`).

- `court_subscriptions` — una fila por cancha (único en `court_id`) con el plan vigente: `plan` (enum `court_plan`: `basico`/`visible`/`agenda`/`pro`, en ese orden — el orden de declaración importa, así `plan >= 'visible'` compara directo sin tabla auxiliar), `current_period_end`, `grace_days` (default 7), `canceled_at`, `price_usd`, `notes`. Es la **fuente de verdad** del plan pagado; se gestiona solo desde `/admin/canchas/[id]/editar` (`setCourtPlan`/`cancelCourtPlan`, `app/actions/billing.ts`), nunca desde el form general de cancha.
- `court_payments` — bitácora append-only de cada cobro manual recibido (`registerCourtPayment`): plan, monto, hasta cuándo cubre, método/referencia. No toca `court_subscriptions` por sí sola — el admin sigue usando `setCourtPlan` para extender el período.
- `court_managers` — vincula un usuario existente como dueño/manager de una cancha (`role`, default `owner`), único por `(court_id, user_id)`. Se gestiona a mano desde `/admin/canchas/[id]/editar` (`addCourtManager`/`removeCourtManager`) — no hay flujo de "reclamar cancha" en la app todavía. Es la base del futuro panel `/mi-cancha` (no construido aún): la RLS de `court_subscriptions`/`court_payments` ya permite `SELECT` a `user_manages_court(court_id)` además de admin.
- **`courts.is_official`/`courts.sponsored_until` son un agregado derivado**, igual patrón que `courts.rating_avg`/`rating_count` vía `recompute_court_rating` sobre `court_ratings`: la función `sync_court_plan_flags(court_id?)` los recalcula a partir de `court_active_plan(court_id)` (plan vigente considerando gracia y cancelación) — `is_official = true` si hay cualquier plan activo, `sponsored_until` se llena solo si el plan es `visible` o superior. Un trigger `court_subscriptions_sync` corre `sync_court_plan_flags` en cada insert/update/delete de `court_subscriptions` (cambios inmediatos); un job de `pg_cron` (`sync-court-plan-flags-daily`, 06:00 UTC) corre `sync_court_plan_flags()` sin argumento todos los días para capturar vencimientos, que no disparan ningún trigger por sí solos. Consecuencia práctica: **todo el resto de la app sigue leyendo `is_official`/`sponsored_until` exactamente igual que antes** (`app/canchas/[id]/page.tsx`, `getOfficialCourts`, `lib/courts/sort.ts`, los pines del mapa) — el gating por plan no tocó ningún componente de lectura, solo quién escribe esos dos campos.
- `lib/billing/plans.ts` es la lógica pura (sin Supabase) espejo de las funciones SQL de arriba: `planAllows(plan, feature)` (catálogo `PLANS`, features por nivel), `subscriptionState(sub, now?)` → `activa`/`en_gracia`/`vencida`/`cancelada`, `isSubscriptionActive`. Testeado en `tests/unit/billing-plans.test.ts`. El enum `basico` quedó deprecado (se fusionó con `visible`, que ahora se etiqueta "Básico" y es el nivel de entrada real): `PLAN_ORDER` conserva las 4 posiciones del enum de Postgres para no romper `plan >= 'visible'`, pero `SELECTABLE_PLAN_ORDER` (`visible`/`agenda`/`pro`) es lo único ofrecible desde el admin — ver `docs/business-model.md` §3.
- Helper de RLS `user_manages_court(p_court_id)` (`SECURITY DEFINER`, mismo patrón que `user_is_group_owner`/`user_is_match_organizer`), creado ya para el futuro panel de dueño aunque hoy nada lo consuma del lado del cliente.
