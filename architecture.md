...
app/
  ...
  auth/confirm/route.ts   Route Handler que recibe el link del email de confirmación y hace verifyOtp server-side (ver sección Autenticación)
...
- **Confirmación de email tras el signup.** `signup` (`app/actions/auth.ts`) llama `supabase.auth.signUp` con `emailRedirectTo: <origin>/auth/confirm?next=/onboarding` (origin sacado del header `Origin` de la request, así funciona igual en local y en prod sin env var). Si no hay sesión inmediata devuelve `{ success: true, message }` y `signup-form.tsx` reemplaza el form por una pantalla de confirmación en vez de seguir mostrando los inputs. `app/auth/confirm/route.ts` es el Route Handler que recibe el click del link del email (`token_hash`/`type`/`next` por query string), hace `supabase.auth.verifyOtp` server-side (así la sesión queda en cookies vía `lib/supabase/server.ts`, que es lo que lee `verifySession()`) y redirige a `next` o a `/login?error=confirmacion` si falla. **Importante:** esto requiere que en el dashboard de Supabase (Authentication → URL Configuration) el `Site URL` y los `Redirect URLs` incluyan el dominio de prod (`https://juegakancha.netlify.app`) — si el `Site URL` sigue en `localhost`, el email de confirmación va a linkear ahí sin importar lo que haga el código. Para que el link del email use esta ruta en vez del `/auth/v1/verify` hosteado por defecto (que redirige con tokens en el hash y nunca llega al server), el template "Confirm signup" en el dashboard debe apuntar a `{{ .SiteURL }}/auth/confirm?token_hash={{ .TokenHash }}&type=signup&next=/onboarding`.
