...
app/
  actions/          Server Actions ("use server") — toda escritura a la DB pasa por aquí
    ...
    courts.ts       agregar/editar cancha (admin, incluye `is_public`); `createPendingCourt` (cualquier usuario) agrega un lugar al instante como cancha `verified: false`; `verifyCourt`/`deletePendingCourt` (admin) verifican o eliminan un pendiente
  ...
  admin/            panel simple (solo `is_admin`), sin nav propia — ve usuarios, partidos, canchas, lugares pendientes de verificar, métricas
