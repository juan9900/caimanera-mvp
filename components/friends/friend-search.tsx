...
import { searchUsersAction, sendFriendRequest } from "@/app/actions/friends";
...
const RELATION_LABEL: Record<UserSearchResult["relation"], string> = {
  ...
  pendiente_recibida: "Te envió solicitud",
  amigos: "Ya son amigos",
};
    ...
    <div>
      ...
      {results.length > 0 && (
        <ul className="mt-3 flex flex-col gap-2">
          {results.map((user) => {
            const relation = sentTo.has(user.id) ? "pendiente_enviada" : user.relation;
            const canAdd = relation === "ninguna";
            return (
              <li
                key={user.id}
