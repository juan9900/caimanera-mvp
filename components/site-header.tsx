...
export async function SiteHeader() {
  ...
  const [matchInvitationCount, friendRequestCount, groupInvitationCount] = session
    ? await Promise.all([
        getMyInvitations().then((invitations) => invitations.length),
        getIncomingFriendRequests().then((requests) => requests.length),
        getMyGroupInvitations().then((invitations) => invitations.length),
      ])
    : [0, 0, 0];
  const invitationCount = matchInvitationCount + groupInvitationCount;
